...
function registerDatabaseHandlers() {
  // Legacy camelCase handlers (still used by preload.ts for documents/stats)
  ...
  ipcMain.handle("db:update-thread-title", async (_, { id, title }) => {
    try {
      dbManager.updateThreadTitle(id, title);
      return { success: true };
    } catch (error) {
      const err = error as Error;
      return { success: false, error: err.message };
...
// ========================================
// RAG Dashboard IPC Handlers
// ========================================

ipcMain.handle("rag:getHealth", async () => {
  try {
    const health = await ragClient.healthCheck(true);
    return health;
  } catch (error) {
    logger.error("Failed to get RAG health", error);
    throw error;
  }
});

ipcMain.handle("rag:getCollectionStats", async () => {
  try {
    return await ragClient.getCollectionStats();
  } catch (error) {
...
ipcMain.handle("rag:getPipelineStats", async () => {
  try {
    const response = await fetch(`${ragClient.getBaseURL()}/pipeline/stats`, {
      method: "GET",
...
ipcMain.handle("rag:clearCollection", async () => {
  try {
    const result = await ragClient.clearCollection();
    // Also reset the local vector store state tracker
    dbManager.updateVectorStoreState(0, 0, {
      clearedAt: Date.now(),
    });
    // Clear all document records from local DB
    const docs = dbManager.getAllDocuments();
    for (const doc of docs) {
      dbManager.deleteDocument(doc.id);
    }
    return result;
  } catch (error) {
...
ipcMain.handle("db:getVectorStoreState", () => {
  try {
    return dbManager.getVectorStoreState();
  } catch (error) {
    logger.error("Failed to get vector store state", error);
    return null;
  }
});
